 ...
 */
const_debug unsigned int sysctl_sched_nr_migrate = 8;	/* EAS: fewer migrations per rebalance */
		   ...
		   int sibling_count_hint)
{
	...
	if (p->nr_cpus_allowed > 1 && !task_is_realtime(p)) /* KernelBumi/5.18: skip if RT pinned */
...
static void __update_child_burst_cache(struct task_struct *p,
					u32 cnt, u32 sum, u64 now)
{
	u8 avg = 0;
	if (cnt)
		avg = sum / cnt;
					   ...
					   u32 depth, u32 *acnt, u32 *asum)
{
	...
	list_for_each_entry(child, &p->children, sibling) {
		...
		update_child_burst_topological(dec, now,
					       depth - 1, &cnt, &sum);
	}
	__update_child_burst_cache(p, cnt, sum, now);
	*acnt += cnt;
...
static inline u8 __inherit_burst_topological(struct task_struct *p, u64 now)
{
	...
	if (child_burst_cache_expired(anc, now))
		update_child_burst_topological(anc, now,
			sched_burst_fork_atavistic - 1, &cnt, &sum);

	return anc->se.child_burst;
}

static void inherit_burst(struct task_struct *p)
{
	u8 burst_cache;
